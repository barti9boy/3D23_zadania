...
void SimpleShapeApplication::init() {
    ...
    */
    ...
    glGenBuffers(1, &lights_buffer_handle);
    ...
    */
    ...
    glBufferData(GL_UNIFORM_BUFFER, sizeof(glm::mat4)*2 + sizeof(glm::mat3), glm::value_ptr(PVM), GL_STATIC_DRAW);
    ...
    OGL_CALL(glBindBuffer(GL_UNIFORM_BUFFER, transformation_buffer_handle));
    glBufferData(GL_UNIFORM_BUFFER, (16+16+12)*sizeof(float), nullptr, GL_STATIC_DRAW);
    glBindBufferBase(GL_UNIFORM_BUFFER, 1, transformation_buffer_handle);
    ...
    glBufferSubData(GL_UNIFORM_BUFFER, 16* sizeof(float), 2* sizeof(float), &VM[0]);
    glBufferSubData(GL_UNIFORM_BUFFER, 20* sizeof(float), 2* sizeof(float), &VM[1]);
    glBufferSubData(GL_UNIFORM_BUFFER, 24* sizeof(float), 4* sizeof(float), &VM[2]);
    glBufferSubData(GL_UNIFORM_BUFFER, 28* sizeof(float), 4* sizeof(float), &VM[3]);
    glBufferSubData(GL_UNIFORM_BUFFER, 32* sizeof(float), 2* sizeof(float), &N[0]);
    glBufferSubData(GL_UNIFORM_BUFFER, 36* sizeof(float), 2* sizeof(float), &N[1]);
    glBufferSubData(GL_UNIFORM_BUFFER, 40* sizeof(float), 4* sizeof(float), &N[2]);
    glBindBuffer(GL_UNIFORM_BUFFER, 0);

    glm::vec3 ambient;
    uintptr_t n_p_lights;
    auto pointLight = new xe::PointLight(glm::vec3(0.0f, 0.0f, 1.0f), glm::vec3(0.0f, 0.0f, 1.0f), 1.0f, 1.0f);
    add_light(*pointLight);

    OGL_CALL(glBindBuffer(GL_UNIFORM_BUFFER, lights_buffer_handle));
    glBufferData(GL_UNIFORM_BUFFER, 4*sizeof(float) + 4*sizeof(float) + 16*sizeof(float), nullptr, GL_STATIC_DRAW);
    glBindBufferBase(GL_UNIFORM_BUFFER, 1, lights_buffer_handle);
    glBufferSubData(GL_UNIFORM_BUFFER, 0* sizeof(float), 3 * sizeof(float), &ambient);
    glBufferSubData(GL_UNIFORM_BUFFER, 4* sizeof(float), sizeof(float), &n_p_lights);
    glBufferSubData(GL_UNIFORM_BUFFER, 8* sizeof(float), 3*sizeof(float), &pointLight->position_in_ws);
    glBufferSubData(GL_UNIFORM_BUFFER, 16* sizeof(float), 3*sizeof(float), &pointLight->color);
    glBufferSubData(GL_UNIFORM_BUFFER, 20* sizeof(float), sizeof(float), &pointLight->intensity);
    glBufferSubData(GL_UNIFORM_BUFFER, 24* sizeof(float), sizeof(float), &pointLight->radius);
...
void SimpleShapeApplication::frame() {
    ...
    auto R = glm::mat3(VM);
    N = glm::mat3(glm::cross(R[1], R[2]), glm::cross(R[2], R[0]), glm::cross(R[0], R[1]));
